...
# === TP / SL ===
TP_PCT    = 0.8   # Take-Profit: 0.8% от цены входа (0 — выключен)
SL_PCT    = 0.0   # Stop-Loss фиксированный (0 — выключен; защита — трейлинг)
TRAIL_PCT = 0.5   # Трейлинг-SL: закрываем если цена откатила на X% от лучшей
    ...
    while True:
        ...
        if scan_pending_signals:
            ...
            if _ranking_already_sent:
                print(f"⏭️ Сигналы для свечи {_cur_4h_hour:02d}:00 UTC уже обработаны — пропускаем", flush=True)
            ...
            if not _ranking_already_sent:
                ...
                if bot_enabled:
                    header = (f"🏆 <b>Рейтинг сигналов — открываем топ {len(to_open)}"
                              f" из {len(ranked)}</b>")
                else:
                ...
                print(f"📊 {len(scan_pending_signals)} сигналов — открываем топ {len(to_open)}", flush=True)

            if _ranking_already_sent:
                print(f"⏭️ Открытие позиций пропущено — сигналы этой свечи уже обработаны", flush=True)
            else:
                # Followup-трекинг только для топ-N сигналов
                for item in to_open:
                    sig = item['signal']
                    sym = item['sym']
                    is_long = sig['signal'] == 'BUY'
