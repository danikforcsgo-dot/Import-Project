"""
elliott_backtest.py — Бэктест Elliott Wave стратегии за последние 7 дней.

Алгоритм:
  1. Загружает 350 x 4H свечей на каждый токен (1 запрос на токен)
  2. Для каждой из 42 временных точек прошлой недели симулирует скан
  3. Записывает найденные сигналы + трекает дальнейшее движение
...
TIMEFRAME      = '4h'
CANDLE_LIMIT   = 350   # 200 контекст + 42 неделя + запас
WEEK_BARS      = 42    # 7 дней × 6 свечей/день
MIN_CONTEXT    = 100   # минимум свечей для поиска волн
ZZ_DEPTH       = 5
MAX_W4_AGE     = 6
MAX_DRIFT      = 0.04
FIB_W2_MIN     = 0.236
...
LEVERAGE       = 15
SCAN_WORKERS   = 12
COOLDOWN_BARS  = 6     # не сигналить по одному токену чаще раза в 6 свечей
...
def backtest_token(token):
    ...
    for offset in range(WEEK_BARS, 0, -1):
        ...
        future = df.iloc[scan_idx + 1:scan_idx + 13]  # до 12 свечей (48ч)
        if future.empty:
            continue
...
def format_report(all_signals):
    ...
    report = (
        f"〽️ <b>Elliott Backtest — 7 дней</b>\n"
        f"⏰ {now_str}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def main():
    now_str = datetime.now(TZ_MOSCOW).strftime('%H:%M  %d.%m.%Y')
    print(f"〽️ Elliott Backtest запущен | {now_str}")
    print(f"   Токенов: {len(TOKENS)} | Период: последние 7 дней (4H)")

    send_telegram(
        f"〽️ <b>Elliott Backtest запущен...</b>\n"
        f"⏰ {now_str}\n"
        f"Сканирую {len(TOKENS)} токенов × 42 свечи — займёт ~2-3 мин"
    )
